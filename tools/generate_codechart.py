...
def render_pinyin_chart(layout_data, key_data, display_names):
    ...
    lines = [
        ...
        '新用户默认采用“双击切换”：单击 F22 显示或隐藏码表，双击切换中英文层一次，松开后保留当前层。'
        '单击最多等待 300 毫秒以区分双击；等待期间开始输入点划会立即确认显隐，仍按当前层输入。'
        '双击保持码表原有显隐状态，长按不触发操作。'
        '也可选择“按住使用”：按住进入拼音层，松开恢复英文，双击只显示或隐藏码表。'
        '操作按键可在设置中修改；升级保留已有“按住使用”设置，原“单击切换”改用新手势。'
